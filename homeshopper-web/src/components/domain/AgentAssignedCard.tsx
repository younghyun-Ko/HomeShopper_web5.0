  ...
  return (
    <GlassCard padding={40} className={cn("mx-auto w-full max-w-[640px]", className)}>
      <div className="flex flex-col items-center text-center">
        ...
        <p className="mt-4 text-[15px] leading-relaxed text-slate">{message}</p>
      </div>

      <div className="mt-8">
        <p className="text-sm font-semibold text-ink">원하는 상담 방식을 선택해주세요</p>
        <div role="radiogroup" aria-label="상담 방식" className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
          {CONSULT_METHODS.map((option) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "rounded-2xl p-[2px] text-left transition-all duration-200",
                  selected ? "bg-grad-primary" : "bg-black/10 hover:bg-black/15",
                )}
              >
                <span className="glass-surface flex flex-col items-center gap-1.5 rounded-[14px] bg-white/70 px-4 py-5 text-center">
                  <option.icon
